        ...
        $.ajaxFileUpload({
            ...
            success: function (data, status)  //服务器成功响应处理函数
            {
                if(data.data.code!=0){
                    ...
                    $.post(get_zhengji_url,{},function(result){
                        if(result.data.code==0){
                            window.location.href = result.data.info+'?attachmentid='+attachmentid + '&aid=' + aid;
                        }else{
    ...
    $(document).on('click', '.add-zhengji-do', function() {
        if($("#desc").val().length>=20){
            ...
            $.post(add_poster_do_url,{'attachmentid':attachmentid,'desc':desc,'aid':aid},function(result){
                window.location.href = activity_url + "?aid=" + aid;
            });
        }
    });
...
$('.voteitem').each(function(index, element) {
