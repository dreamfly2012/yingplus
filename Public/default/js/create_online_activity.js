$(document).ready(function(){
    ...
    $(document).on('change',"#type",function(){
        var type = $(this).val();
        if(type==1){
            $(".vote_div").removeClass('imp_none');
        }else{
            $(".vote_div").addClass('imp_none');
        }
    });
    //添加选项
    ...
    $(document).on('click',".delete_option",function(){
        if($(".delete_option").length==3){
            $(this).parent('div.selectitems').remove();
    ...
    $(document).on('change','.vote_image',function(){
        ...
        $.ajaxFileUpload({
            url: upload_vote_image_url, //用于文件上传的服务器端请求地址
    ...
    $(document).on("click",".create-online-activity-submit",function(){
        ...
        if($("#title").val()==''||$('#desc').val()==''){
            ...
            if($('#encrollend_date').val()=='custom' && $("#encrollend_custom_date").val()==''){
                // alert('日期必须填写');
                // return false;
            }else{
                //问题验证
